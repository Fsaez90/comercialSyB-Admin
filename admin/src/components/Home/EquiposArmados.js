...
function EquiposArmados({render, setRender, date, clock}) {
...
const getData = async () => {
  try {
    setLoading(true)
    const result = await fetch('https://comercialsyb-backend-production.up.railway.app/comercial/equipos_armados/')
  ...
  try {
    const response = await fetch(`https://comercialsyb-backend-production.up.railway.app/comercial/update/${n}/`, {
      ...
      body: JSON.stringify({
        ...
        hora_trabajo: clock,
        revisado: true,
        status: "Equipo armado con ppto rechazado listo para retiro (Cliente notificado).",
        terminada: true,
