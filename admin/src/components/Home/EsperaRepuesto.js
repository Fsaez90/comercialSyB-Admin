...
function EsperaRepuesto({render, setRender, setAdminEsp, adminEsp}) {
  ...
  const getData = async () => {
    try {
      setLoading(true)
      const result = await fetch('https://comercialsyb-backend-production.up.railway.app/comercial/esp_repuesto/')
